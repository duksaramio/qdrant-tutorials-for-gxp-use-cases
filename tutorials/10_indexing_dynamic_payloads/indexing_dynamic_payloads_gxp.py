...
Target Environment: Local Qdrant server at http://localhost:6333 + Langfuse at http://localhost:3000
"""
...
# ---------------------------------------------------------------------------
# 1. Connect to Local Qdrant & Initialize Ollama Client & Langfuse
# ---------------------------------------------------------------------------
print("=" * 80)
...
# ---------------------------------------------------------------------------
@observe(as_type="span", name="ingest-dynamic-eav-records")
def ingest_dynamic_records():
    raw_records = [
        {
    ...
    langfuse.update_current_span(
        output={"indexed_points": len(points), "collection": COLLECTION_NAME},
    )
    return len(points)
...
count = ingest_dynamic_records()
print(f"Indexed {count} points containing {count} reshaped dynamic payload structures.")


# ---------------------------------------------------------------------------
# 5. Query Patterns on Dynamic Payloads
# ---------------------------------------------------------------------------
@observe(as_type="retriever", name="dynamic-exact-filter-query")
def query_pattern_1():
    print("\n" + "=" * 80)
    ...
    langfuse.update_current_span(
        input={"pattern": "exact_attrs_flat", "filters": ["detector_type=UV-Vis", "instrument_id=HPLC-QC-04"]},
        output=[{"doc_id": h.payload["doc_id"], "title": h.payload["title"]} for h in hits_flat],
    )
    return hits_flat


@observe(as_type="retriever", name="dynamic-numeric-range-query")
def query_pattern_2():
    print("\n" + "=" * 80)
    ...
    langfuse.update_current_span(
        input={"pattern": "numeric_range", "criteria": "flow_rate >= 1.0 & column_temp <= 40.0"},
        output=[{"doc_id": h.payload["doc_id"], "numeric_attrs": h.payload["attrs_num"]} for h in hits_numeric],
    )
    return hits_numeric


@observe(as_type="retriever", name="dynamic-hybrid-dr-bounds-query")
def query_pattern_3():
    print("\n" + "=" * 80)
    ...
    langfuse.update_current_span(
        input={"query": query_text, "bounds": "RTO <= 4h & RPO <= 15m & is_gxp=True"},
        output=[{"doc_id": h.payload["doc_id"], "score": h.score} for h in hits_semantic_filtered],
    )
    return hits_semantic_filtered


@observe(name="tutorial-10-dynamic-payloads-pipeline")
def execute_dynamic_scenarios():
    query_pattern_1()
    query_pattern_2()
    query_pattern_3()
...
trace_url = execute_dynamic_scenarios()
langfuse.flush()
